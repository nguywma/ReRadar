...
class REM(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"[Forward pass] Inference time: {inference_time*1000:.2f} ms")
...
# class REIN(nn.Module):
#     def __init__(self, csv_path="rein_inference_time.csv"):
#         super(REIN, self).__init__()
#         self.rem = REM()
#         self.pooling = NetVLAD()
...
#                 writer = csv.writer(f)
#                 writer.writerow(["timestamp", "inference_time_ms"])
...
#             writer = csv.writer(f)
#             writer.writerow([time.time(), inference_time_ms])
