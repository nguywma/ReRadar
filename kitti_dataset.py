...
class InferDataset(data.Dataset):
    def __init__(self, seq, dataset_path = './datasets/datasets/KITTI/',sample_inteval=1):
        super().__init__()
...
#             else:
#                 fn += 1  # False negative: should have matched but did not
#         else:
#             if pred[0] in positives:
#                 fp += 1  # False positive: incorrect match
#             else:
#                 tn += 1  # True negative: correctly not matching anything
...
def collate_fn(batch):
    ...
    def prepare_tensor(img):
        ...
        # Ensure shape is [C, H, W] for interpolation
        if img.dim() == 3 and (img.shape[1] != H or img.shape[2] != W):
            img = F.interpolate(img.unsqueeze(0), size=(H, W), 
                                mode='bilinear', align_corners=False).squeeze(0)
        return img

    # Process all elements
    query = torch.stack([prepare_tensor(q) for q in query], 0)
    positive = torch.stack([prepare_tensor(p) for p in positive], 0)
    
    # Negatives are usually already tensors from TrainingDataset, 
    # but we check their spatial dimensions just in case
    resized_negs = []
    for neg_batch in negatives:
        if neg_batch.shape[2] != H or neg_batch.shape[3] != W:
            neg_batch = F.interpolate(neg_batch, size=(H, W), 
                                      mode='bilinear', align_corners=False)
        resized_negs.append(neg_batch)

    negatives = torch.cat(resized_negs, 0)
