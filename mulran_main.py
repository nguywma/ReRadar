...
def train_epoch(epoch, model, train_set):
    ...
    if epoch >= 0:
        ...
        if isinstance(train_set, ConcatDataset):
            for d in train_set.datasets: d.mining = False
        else:
            train_set.mining = False

        # Use mulran_dataset.collate_fn here
        training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, 
            batch_size=opt.batchSize, shuffle=False, 
            collate_fn=mulran_dataset.collate_fn) 
            
        # ... (Global descriptor extraction code)

        # Refresh cache for all sub-datasets
        if isinstance(train_set, ConcatDataset):
            for d in train_set.datasets:
                d.mining = True
                d.cache = join(opt.cachePath, 'train_feat_cache.hdf5')
                d.refreshCache()
        else:
    ...
    training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, 
                ...
                collate_fn=mulran_dataset.collate_fn) # Fixed collate_fn
    model.train()
...
def train_epoch_infonce(epoch, model, train_set):
    ...
    # Initialize InfoNCE. 
    # Use 'paired' mode because each query in your loader has its own specific set of negatives.
    criterion = InfoNCE(temperature=0.1, reduction='mean', negative_mode='paired').to(device)
    
    model.eval()

    # 1. Cache Building (Same logic as Triplet, ensures hard mining is updated)
    if epoch >= 0:
        print('====> Building Cache for Hard Mining')
        sub_datasets = train_set.datasets if isinstance(train_set, ConcatDataset) else [train_set]
        for d in sub_datasets:
            d.mining = False

        cache_file = join(opt.cachePath, 'train_feat_cache.hdf5')
        with h5py.File(cache_file, mode='w') as h5: 
            pool_size = model.global_feat_dim
            h5feat = h5.create_dataset("features", [len(train_set), pool_size], dtype=np.float32)
            cache_loader = DataLoader(dataset=train_set, num_workers=opt.threads, 
                                    batch_size=opt.cacheBatchSize, shuffle=False, 
                                    collate_fn=kitti_dataset.collate_fn)

            with torch.no_grad():
                for _, (query, _, _, indices) in enumerate(tqdm(cache_loader, leave=False)):
                    query = query.to(device)
                    _, _, global_descs = model(query)
                    indices_np = indices.numpy()
                    descs_np = global_descs.detach().cpu().numpy()
                    sort_idx = np.argsort(indices_np)
                    h5feat[indices_np[sort_idx], :] = descs_np[sort_idx, :]

        for d in sub_datasets:
            d.mining = True
            d.refreshCache(cache_file)
    ...
    for iteration, (query, positives, negatives, indices) in enumerate(training_data_loader):
        # B: Batch Size, C: Channels, H/W: Image dims
        B = query.shape[0]
        
        # Reshape negatives from [B*N, C, H, W] to [B, N, C, H, W] 
        # to identify which negatives belong to which query
        num_negs = negatives.shape[0] // B
        
        # Forward pass for all images in one batch
        input_data = torch.cat([query, positives, negatives]).to(device)
        _, _, global_descs = model(input_data)

        # Split back to Query, Positive, and flattened Negatives
        q_feat, p_feat, n_feat_flat = torch.split(global_descs, [B, B, negatives.shape[0]])
        
        # Reshape n_feat to (N, M, D) for InfoNCE 'paired' mode
        # N = Batch Size, M = Number of negatives per query, D = Descriptor Dim
        n_feat = n_feat_flat.view(B, num_negs, -1)

        optimizer.zero_grad()

        # Calculate InfoNCE Loss
        # info_nce(query, positive_key, negative_keys)
        loss = criterion(q_feat, p_feat, n_feat)
        ...
        if iteration % 50 == 0:
            print("==> Epoch[{}]({}/{}): Loss: {:.4f}".format(epoch, iteration, 
                n_batches, batch_loss), flush=True)
            writer.add_scalar('Train/Loss', batch_loss, (epoch * n_batches) + iteration)
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        for epoch in range(opt.nEpochs):
            if opt.loss.lower() == 'triplet':
                train_epoch(epoch, model, train_set)
            elif opt.loss.lower() == 'infonce':

                train_epoch_infonce(epoch, model, train_set)
